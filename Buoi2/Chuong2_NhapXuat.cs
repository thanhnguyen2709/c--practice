    ...
    class Chuong2_NhapXuat
    {
        ...
        static void RunTests()
        {
            ...
            Console.WriteLine("--- Test Bai 2: Hinh tron ---");
            {
                ...
                double P = 2 * PI * R;
                total += 2;
                bool dungS = (Math.Abs(S - 78.5) < 0.1);
                bool dungP = (Math.Abs(P - 31.4) < 0.1);
                if (dungS) pass++;
                if (dungP) pass++;
                Console.WriteLine("  Test 2.1: R=5 => S={0:F1} (mong doi: 78.5) => {1}", S, dungS ? "PASS" : "FAIL");
                Console.WriteLine("  Test 2.2: R=5 => P={0:F1} (mong doi: 31.4) => {1}", P, dungP ? "PASS" : "FAIL");
            }
            {
                double R = 10, PI = 3.14;
                double S = PI * R * R;
                double P = 2 * PI * R;
                total += 2;
                bool dungS = (Math.Abs(S - 314) < 0.1);
                bool dungP = (Math.Abs(P - 62.8) < 0.1);
                if (dungS) pass++;
                if (dungP) pass++;
                Console.WriteLine("  Test 2.3: R=10 => S={0:F1} (mong doi: 314.0) => {1}", S, dungS ? "PASS" : "FAIL");
                Console.WriteLine("  Test 2.4: R=10 => P={0:F1} (mong doi: 62.8) => {1}", P, dungP ? "PASS" : "FAIL");
            }
            ...
            Console.WriteLine("--- Test Bai 3: Doi sang giay ---");
            {
                ...
                int tongGiay = h * 3600 + m * 60 + s;
                total++;
                bool dung = (tongGiay == 4810);
                if (dung) pass++;
                Console.WriteLine("  Test 3.1: 1h 20m 10s = {0} giay (mong doi: 4810) => {1}",
                    tongGiay, dung ? "PASS" : "FAIL");
            }
            {
                int h = 0, m = 0, s = 0;
                int tongGiay = h * 3600 + m * 60 + s;
                total++;
                bool dung = (tongGiay == 0);
                if (dung) pass++;
                Console.WriteLine("  Test 3.2: 0h 0m 0s = {0} giay (mong doi: 0) => {1}",
                    tongGiay, dung ? "PASS" : "FAIL");
            }
            {
                int h = 2, m = 30, s = 45;
                int tongGiay = h * 3600 + m * 60 + s;
                total++;
                bool dung = (tongGiay == 9045);
                if (dung) pass++;
                Console.WriteLine("  Test 3.3: 2h 30m 45s = {0} giay (mong doi: 9045) => {1}",
                    tongGiay, dung ? "PASS" : "FAIL");
            }
            ...
            Console.WriteLine("--- Test Bai 4: Doi sang gio phut giay ---");
            {
                ...
                total++;
                bool dung = (gio == 1 && phut == 20 && giay == 10);
                if (dung) pass++;
                Console.WriteLine("  Test 4.1: 4810s = {0}:{1}:{2} (mong doi: 1:20:10) => {3}",
                    gio, phut, giay, dung ? "PASS" : "FAIL");
            }
            {
                int t = 3661;
                int gio = t / 3600, phut = (t % 3600) / 60, giay = t % 60;
                total++;
                bool dung = (gio == 1 && phut == 1 && giay == 1);
                if (dung) pass++;
                Console.WriteLine("  Test 4.2: 3661s = {0}:{1}:{2} (mong doi: 1:1:1) => {3}",
                    gio, phut, giay, dung ? "PASS" : "FAIL");
            }
            {
                int t = 59;
                int gio = t / 3600, phut = (t % 3600) / 60, giay = t % 60;
                total++;
                bool dung = (gio == 0 && phut == 0 && giay == 59);
                if (dung) pass++;
                Console.WriteLine("  Test 4.3: 59s = {0}:{1}:{2} (mong doi: 0:0:59) => {3}",
                    gio, phut, giay, dung ? "PASS" : "FAIL");
            }
        ...
        public static void Main(string[] args)
        {
            ...
            Console.Write("Chon bai tap (0-4): ");
